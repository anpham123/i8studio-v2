...
export default async function SolutionPage() {
  ...
  const works = await prisma.work.findMany({
    where: { image: { not: "" } },
    orderBy: { order: "asc" },
    select: { title: true, titleJa: true, image: true, type: true },
  });

  // Group works by type → { still: [...], animation: [...], ... }
  const worksByType: Record<string, { title: string; titleJa: string; image: string }[]> = {};
  for (const w of works) {
    const t = w.type || "still";
    if (!worksByType[t]) worksByType[t] = [];
    worksByType[t].push({ title: w.title, titleJa: w.titleJa, image: w.image });
  }

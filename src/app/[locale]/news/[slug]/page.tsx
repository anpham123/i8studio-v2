...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const post = await prisma.post.findUnique({
    where: { slug: params.slug, status: "PUBLISHED", category: "NEWS" },
  });
  if (!post) return {};
...
export default async function NewsArticlePage({ params }: Props) {
  ...
  const post = await prisma.post.findUnique({
    where: { slug, status: "PUBLISHED", category: "NEWS" },
  });
  if (!post) notFound();
  ...
  return (
    <main className="min-h-screen bg-[#0a0a0f] pt-28 pb-20">
      ...
      <article className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {excerpt && (
          <p className="text-white/60 text-lg leading-relaxed mb-8 border-l-2 border-blue-500/50 pl-4">
            {excerpt}
          </p>
        )}

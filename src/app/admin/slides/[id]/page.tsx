...
export default function EditSlidePage() {
  ...
  useEffect(() => {
    fetch(`/api/slides/${id}`).then((r) => r.json()).then((d) => {
  ...
  const save = async () => {
    setSaving(true);
    const res = await fetch(`/api/slides/${id}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...form, order: parseInt(String(form.order)) || 0 }) });
    const data = await res.json();
    setSaving(false);
    if (data.data) toast("Đã lưu", "success"); else toast("Lỗi", "error");
  };

  if (loading) return <AdminShell title="Slide"><div className="flex justify-center py-24"><Loader2 className="animate-spin text-blue-500" size={32} /></div></AdminShell>;

  return (
    <AdminShell title="Chỉnh sửa Slide" actions={<div className="flex gap-2"><button onClick={() => setShowDel(true)} className="border border-red-200 text-red-600 px-3 py-2 rounded-lg text-sm hover:bg-red-50"><Trash2 size={15} /></button><button onClick={save} disabled={saving} className="flex items-center gap-2 bg-blue-600 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-blue-700 disabled:opacity-50"><Save size={15} /> Lưu</button></div>}>
      <div className="max-w-2xl space-y-5">
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6 space-y-4">
          ...
          <div><label className="block text-sm font-medium text-gray-700 mb-1.5">Gradient CSS classes</label><input value={String(form.gradient || "")} onChange={(e) => set("gradient", e.target.value)} className="w-full border border-gray-200 rounded-lg px-4 py-2.5 text-sm font-mono focus:outline-none" /></div>
          <div className="grid grid-cols-2 gap-4">
        ...
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
          <ImageUpload label="Ảnh nền slide" value={String(form.image || "")} onChange={(url) => set("image", url)} />
        </div>
      </div>
      <ConfirmDialog open={showDel} message={`Xóa slide "${form.title}"?`} onConfirm={async () => { await fetch(`/api/slides/${id}`, { method: "DELETE" }); router.push("/admin/slides"); }} onCancel={() => setShowDel(false)} />
    </AdminShell>
  );
}

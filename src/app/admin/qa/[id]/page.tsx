...
export default function EditQAPage() {
  ...
  useEffect(() => {
    fetch(`/api/qa/${id}`).then((r) => r.json()).then((d) => {
    ...
    <AdminShell title="Chỉnh sửa Q&A" actions={<div className="flex gap-2"><button onClick={() => setShowDel(true)} className="border border-red-200 text-red-600 px-3 py-2 rounded-lg text-sm hover:bg-red-50"><Trash2 size={15} /></button><button onClick={save} disabled={saving} className="flex items-center gap-2 bg-blue-600 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-blue-700 disabled:opacity-50"><Save size={15} /> Lưu</button></div>}>
      <div className="max-w-2xl space-y-5">
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6 space-y-4">
          ...
          <div><label className="block text-sm font-medium text-gray-700 mb-1.5">Thứ tự</label><input type="number" value={form.order || "0"} onChange={(e) => set("order", e.target.value)} className="w-32 border border-gray-200 rounded-lg px-4 py-2.5 text-sm focus:outline-none" /></div>
        </div>
      </div>
      <ConfirmDialog open={showDel} message={`Xóa câu hỏi này?`} onConfirm={async () => { await fetch(`/api/qa/${id}`, { method: "DELETE" }); router.push("/admin/qa"); }} onCancel={() => setShowDel(false)} />
    </AdminShell>
  );
}

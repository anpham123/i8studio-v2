...
export default function EditCompositeExamplePage() {
  ...
  // Fetch initial data
  useEffect(() => {
    fetch(`/api/composite-examples/${id}`)
      .then((r) => r.json())
      .then((d) => {
        if (d.data) {
          setForm({
            ...d.data,
            titleJp: d.data.titleJp || "",
            location: d.data.location || "",
            order: String(d.data.order),
          });
        }
        setLoading(false);
      })
  ...
  const save = async () => {
    ...
    setSaving(true);
    try {
      const res = await fetch(`/api/composite-examples/${id}`, {
        method: "PUT",
      ...
      if (res.ok && data.data) {
        ...
        toast(data.error || "Thất bại khi cập nhật", "error");
      }
    } catch {
      setSaving(false);
  ...
  const handleDelete = async () => {
    setDeleting(true);
    try {
      const res = await fetch(`/api/composite-examples/${id}`, { method: "DELETE" });
      setDeleting(false);
      if (res.ok) {
  ...
  if (loading) {
    return (
      <AdminShell title="Chỉnh sửa Photo Composite">
  ...
  return (
    <AdminShell
      title="Chỉnh sửa Photo Composite"
      actions={
    ...
    >
      ...
      <ConfirmDialog
        open={showDelete}
        message={`Xóa ảnh ghép "${form.title}"?`}
